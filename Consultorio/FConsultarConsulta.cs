...
{
    public partial class FConsultarConsulta : Form
    {
        public int indice = 0, vtieneparametro = 0;
        public string valorparametro = "";
        CNconsulta cNconsulta = new CNconsulta ();
        public FConsultarConsulta()
        {
            InitializeComponent();
        }

        private void FConsultarConsulta_Load(object sender, EventArgs e)
        {
        ...
        {
            RConsultaParametro rConsultaParametro = new RConsultaParametro();
            rConsultaParametro.parametro = tbBuscar.Text;
            rConsultaParametro.ShowDialog();
        }
        ...
        {
            valorparametro = tbBuscar.Text.Trim();
            if (cNconsulta.ObtenerConsulta(valorparametro) != null)
            {
                DGVdatos.DataSource = cNconsulta.ObtenerConsulta(valorparametro);
                //Se ejecuta elmétodo para mostrar los datos
                DGVdatos.Columns[0].Width = 80; //IDConsulta
                DGVdatos.Columns[1].Width = 200; //Id_Paciente
                DGVdatos.Columns[2].Width = 225; //Motivo
                DGVdatos.Columns[3].Width = 100; //Enfermedades
                DGVdatos.Columns[4].Width = 125; //Sintomas
                DGVdatos.Columns[5].Width = 125; //Antecedentes
                DGVdatos.Columns[6].Width = 150; //Fecha
                DGVdatos.Columns[7].Width = 100; //Diagnostico
